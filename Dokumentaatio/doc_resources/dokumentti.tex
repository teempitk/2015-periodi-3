...
Kun pakkausohjelmalle annetaan ajettaessa parametri {\tt -p}, ohjelma tulostaa tietoa suorituksen etenemisestä ja kuhunkin vaiheeseen kuluneesta ajasta. Pakataan nyt seuraavat tiedostot:
\begin{itemize}
\item {\tt alice.txt}
...
\caption{Pakkausaika tiedoston koon funktiona. Huffman punaisella, BWT vihreällä, MTF sinisellä ja kokonaisaika mustalla.}
\label{pakkauskuva}
\end{figure}

Kuten kuvaajasta \ref{pakkauskuva} havaitaan, teoriassa todetut aikavaativuudet vaikuttavat pätevän tämän testin nojalla myös käytännössä. Näin pienellä otoksella on tietenkin mahdotonta sanoa mitään varmaksi, ja esim. luokkien $\mathcal{O}(n\log n)$ ja $\mathcal{O}(n)$ eroa on lähes mahdotonta havaita.
...
Kuten kuvasta \ref{purkukuva} nähdään, Huffman-koodaus ja MTF vaikuttavat noudattavan lineaarista aikavaativuutta. BWT:tä täytyy tarkastella erikseen, sillä sen käyttämä aika on tässä niin pieni, että vaikutus hukkuu. Kuvaajasta \ref{bwtpurkukuva} voidaan nähdä aavistus $\mathcal{O}(n\log n)$-operaation suoritusajan loivasti kiihtyvästä kasvusta. Aikavaativuuden saa paremmin esiin, kun ottaa mukaan vielä laajemman skaalan eri tiedostokokoja. Kuvaajassa \ref{purkuraamatunkanssa} dataan on vielä lisätty raamatun \cite{bible} purku (koko pakattuna 1 213 787 tavua, kesto 42,938 sekuntia). Dataan on sovitettu nyt käyrät muotoa $f(n)=an+b$ (lineaarinen) ja $f(n)=an\log_2 (n) + bn + c$ ($\in \mathcal{O}(n\log n)$). Kuten havaitaan, jälkimmäinen sopii dataan huomattavasti paremmin.

\begin{figure}[ht!]
\centering
\includegraphics[width=120mm]{purkutraamatunkanssa.png}
\caption{Funktioiden sovittamista purkamisen kokonaissuoritusaikadataan, johon on lisätty vielä huomattavasti suurempi testitiedosto. Lineaarinen funktio vihreällä, $\mathcal{O}(n\log n)$-funktio sinisellä.}
